        ...
        <v-card class="elevation-5"   >
          <v-col cols="12" class="text-center">
                    <v-avatar size="100" color="#f2e8df">
                      <v-img
                    src="../../assets/images/placement_logo.png"
                    alt="Profile"
                    width="100%"
                  ></v-img>
                    </v-avatar>
          ...
          <v-card-subtitle class="white--text text-center">
            {{ placementView.position }}
          </v-card-subtitle>
          <v-card-subtitle class="white--text text-center">
            {{ placementView.placedAt }}
          </v-card-subtitle>
          <v-card-subtitle class="white--text text-center">
            {{ placementView.placedDate }}
          </v-card-subtitle>
          <v-card-text>
      ...
      axios({
        ...
        .then((response) => {
          if (response.data.status == true) {
            ...
            // this.loading = false;
            this.placementView = response.data.review;
          } else {
            this.appLoading = false;
            this.msg = response.data.msg;
            this.snackbar = true;
